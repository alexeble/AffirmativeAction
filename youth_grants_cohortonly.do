...
graph export "$figure_path/Fig_young_grant_1966_1975_CNKI_NSFC.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_CNKI_NSFC.pdf", replace	


/* ----------------------------------------------------------- */
/*         youth grants: female cohorts [1970, 1971]           */
/* ----------------------------------------------------------- */
...
*reghdfe youth_grant_ry `treatlist1' `controls' if `cond1', absorb(author_numid2 year) vce(cluster author_numid2) // collinearity and two few obs for each cluster: 2
reghdfe youth_grant_ry `treatlist1' `controls' if `cond1', absorb(author_numid2 year) vce(robust) 
est store grant1
...
graph export "$figure_path/Fig_young_grant_female_1970_1971.eps", replace
graph export "$figure_path/Fig_young_grant_female_1970_1971.pdf", replace	
...
coefplot (grant1, msymbol(O) mcolor(gs0) lcolor(gs0) ciopts(recast(rcap) lcolor(gs0) lpattern(solid)) offset(-0.1)) ///
	...
	coeflabels(years_n10="-10" years_n9=" " years_n8=" " years_n7=" " years_n6=" " years_n5="-5" years_n4=" " ///
		...
		years_p5="5" years_p6=" " years_p7=" " years_p8=" " years_p9=" " years_p10="10") ///
	omitted ///
	legend(order(2 "Fields with more female scientists" 4 "Fields with fewer female scientists") pos(6) region(lw(0)) cols(2) symxsize(*1.5) size(4)) ///
	xlabel(,labsize(4)) ///
	ylabel(-0.05(0.05)0.1,labsize(4)) ///
	xtitle("Years relative to policy start (2011)", size(4)) ///
	ytitle("Won the young scientist grant", size(4))

graph export "$figure_path/Fig_young_grant_1966_1975_field_het.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_field_het.pdf", replace	
...
graph export "$figure_path/Fig_young_grant_1966_1975_ability_het.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_ability_het.pdf", replace	
...
graph export "$figure_path/Fig_young_grant_1966_1975_ability_hettoppub.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_ability_het.pdf", replace	


/* ---------------------------------------------------- */
/*         youth grants: cohorts [1966, 1975]           */
/*           High-tier vs. lower-tier univs             */
/* ---------------------------------------------------- */
...
local treatlist1 years_n10 years_n9 years_n8 years_n7 years_n6 years_n5 years_n4 years_n3 years_n2 year_cutoff years_p0 ///
  years_p1 years_p2 years_p3 years_p4 years_p5 years_p6 years_p7 years_p8 years_p9 years_p10 
local cond1 author_birthyr >= 1966 & author_birthyr <= 1975
local cond2_1 (wkunit_type == 1 | wkunit_type == 2 | wkunit_type == 5) 
local cond2_2 (wkunit_type != 1 & wkunit_type != 2 &  wkunit_type != 5) 
local controls treat_female i.female#i.year i.treated#i.year
...
graph export "$figure_path/Fig_young_grant_1966_1975_3rank_het.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_3rank_het.pdf", replace	


/* ------------------------------------------------------------------ */
/*                   youth grants: cohorts [1966, 1975]               */
/*    top- vs. lower-tier univs + high- vs. Lower-performing scholars    */
/* ------------------------------------------------------------------ */
...
reghdfe youth_grant_ry `treatlist1' `controls' if `cond1' & `cond2_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant2
reghdfe youth_grant_ry `treatlist1' `controls' if `cond1' & `cond3_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant3
reghdfe youth_grant_ry `treatlist1' `controls' if `cond1' & `cond3_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant4
...
coefplot (grant1, msymbol(O) mcolor(gs0) lcolor(gs0) ciopts(recast(rcap) lcolor(gs0) lpattern(solid)) offset(-0.22)) ///
	...
	legend(order(2 "High-performing scholars + Elite univ." 4 "High-performing scholars + Other univ." ///
		6 "Lower-performing scholars + Elite univ." 8 "Lower-performing scholars + Other univ.") ///
		pos(6) region(lw(0)) cols(2) symxsize(*1.5)) ///
	xlabel(,labsize(4)) ///
	ylabel(-0.05(0.05)0.1,labsize(4)) ///
	xtitle("Years relative to policy start (2011)", size(4)) ///
	ytitle("Won the young scientist grant", size(4))

graph export "$figure_path/Fig_young_grant_1966_1975_univ_ability_het.eps", replace
graph export "$figure_path/Fig_young_grant_1966_1975_univ_ability_het.pdf", replace	
...
/*
sum youth_grant_ry if `cond1'

sum youth_grant_ry if `cond1' & `cond2_1' 
sum youth_grant_ry if `cond1' & `cond2_2'
sum youth_grant_ry if `cond1' & `cond3_1' 
sum youth_grant_ry if `cond1' & `cond3_2'

sum youth_grant_ry if `cond1' & `cond2_1' & `cond3_1'
sum youth_grant_ry if `cond1' & `cond2_2' & `cond3_1'
sum youth_grant_ry if `cond1' & `cond2_1' & `cond3_2'
sum youth_grant_ry if `cond1' & `cond2_2' & `cond3_2'

reghdfe youth_grant_ry treat_female_post `controls' if `cond1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg1

reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg2
reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg3

reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond3_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg4
reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond3_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg5


reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_1' & `cond3_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg2
reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_2' & `cond3_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg3

reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_1' & `cond3_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg2
reghdfe youth_grant_ry treat_female_post `controls' if `cond1' & `cond2_2' & `cond3_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store reg3
*/

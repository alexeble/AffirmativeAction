...
graph export "$figure_path/Fig_general_grant_1966_1975.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975.pdf", replace	
...
graph export "$figure_path/Fig_general_grant_1966_1975_CNKI_NSFC.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975_CNKI_NSFC.pdf", replace	


/* ----------------------------------------------------------- */
/*         general grants: female cohorts [1970, 1971]         */
/* ----------------------------------------------------------- */
...
reghdfe gen_grant_ry `treatlist1' if `cond1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant1
...
graph export "$figure_path/Fig_general_grant_female_1970_1971.eps", replace
graph export "$figure_path/Fig_general_grant_female_1970_1971.pdf", replace	
...
local treatlist1 years_n10 years_n9 years_n8 years_n7 years_n6 years_n5 years_n4 years_n3 years_n2 year_cutoff years_p0 ///
  years_p1 years_p2 years_p3 years_p4 years_p5 years_p6 years_p7 years_p8 years_p9 years_p10 
local cond1 author_birthyr >= 1966 & author_birthyr <= 1975
local cond2_1 (nsfc_field0 == 1 | nsfc_field0 == 4 | nsfc_field0 == 5 | nsfc_field0 == 6)
local cond2_2 (nsfc_field0 == 2 | nsfc_field0 == 3 | nsfc_field0 == 8) 
local controls treat_female i.female#i.year i.treated#i.year
...
graph export "$figure_path/Fig_general_grant_1966_1975_field_het.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975_field_het.pdf", replace	
...
graph export "$figure_path/Fig_general_grant_1966_1975_rank_het.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975_rank_het.pdf", replace	
...
reghdfe gen_grant_ry `treatlist1' `controls' if `cond1' & `cond2_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant2
reghdfe gen_grant_ry `treatlist1' `controls' if `cond1' & `cond2_3', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant3
...
graph export "$figure_path/Fig_general_grant_1966_1975_3rank_het.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975_3rank_het.pdf", replace	
...
reghdfe gen_grant_ry `treatlist1' `controls' if `cond1' & `cond2_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant2
reghdfe gen_grant_ry `treatlist1' `controls' if `cond1' & `cond3_1', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant3
reghdfe gen_grant_ry `treatlist1' `controls' if `cond1' & `cond3_2', absorb(author_numid2 year) vce(cluster author_numid2) 
est store grant4
...
coefplot (grant1, msymbol(O) mcolor(gs0) lcolor(gs0) ciopts(recast(rcap) lcolor(gs0) lpattern(solid)) offset(-0.22)) ///
	...
	coeflabels(years_n10="-10" years_n9=" " years_n8=" " years_n7=" " years_n6=" " years_n5="-5" years_n4=" " ///
		...
		years_p5="5" years_p6=" " years_p7=" " years_p8=" " years_p9=" " years_p10="10") ///
	omitted ///
	legend(order(2 "High-performing scholars + Elite universities" 4 "High-performing scholars + Other universities" ///
		6 "Lower-performing scholars + Elite universities" 8 "Lower-performing scholars + Other universities") ///
...
graph export "$figure_path/Fig_general_grant_1966_1975_univ_ability_het.eps", replace
graph export "$figure_path/Fig_general_grant_1966_1975_univ_ability_het.pdf", replace	


/* ---------------------------------------------------- */
/*       general grants: cohorts [1966, 1975]           */
/*              table: one post-period                  */
/* ---------------------------------------------------- */
...
texdoc init "$table_path/Tab_general_grants.tex", replace force
...
forvalues i = 1/7 {
	...
	* Saving the parameter estimates
    local te1_`i' = _b[treat_female_post1]
    local se1_`i' = _se[treat_female_post1]
    local te2_`i' = _b[treat_female_post2]
    local se2_`i' = _se[treat_female_post2]
	
    * Generating t-stats
    local tstat1_`i' = abs(_b[treat_female_post1]/_se[treat_female_post1])		
    local tstat2_`i' = abs(_b[treat_female_post2]/_se[treat_female_post2])		
...
forvalues i = 1/7 {
	...
	* Generating stars for significance
    ...
    if `tstat1_`i'' >= 1.96 local st1_`i' = "**"
    if `tstat1_`i'' >= 2.33 local st1_`i' = "***"
    if `tstat2_`i'' >= 1.64 local st2_`i' = "*"
    if `tstat2_`i'' >= 1.96 local st2_`i' = "**"
    if `tstat2_`i'' >= 2.33 local st2_`i' = "***"
	
    * Formatting the parameters (making them the same number of significant figures and pretty for the table)
    local te1_`i': di %9.3f `te1_`i''
    local se1_`i': di %9.3f `se1_`i''
    local te2_`i': di %9.3f `te2_`i''
    local se2_`i': di %9.3f `se2_`i''
	
    * Dropping the hanging space that this code generates so I can use parentheses around the parameter without it appearing like "(x.xxx )"
    local se1_`i' = subinstr("`se1_`i''"," ","",.)			
    local se2_`i' = subinstr("`se2_`i''"," ","",.)		
	
}	

local tex_te1 "Policy effect: [2011, 2015]"
local tex_se1 ""
local tex_te2 "Policy effect: [2016, 2021]"
local tex_se2 ""
local tex_mean "\textit{Outcome mean}"
local tex_obs "\textit{N}"
...
forvalues i = 1/7 {
    ...
    local tex_te2 = "`tex_te2' & `te2_`i''`st2_`i''"
    local tex_se2 = "`tex_se2' & (`se2_`i'')"
    local tex_mean = "`tex_mean' & `mean_`i''"
    local tex_obs = "`tex_obs' & `n_`i''"	
